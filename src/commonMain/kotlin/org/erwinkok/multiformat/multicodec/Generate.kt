...
import java.util.TreeMap
    ...
    companion object {
        ...
        fun codeToName(code: Int): Result<String> {
            val name = codeToName[code] ?: return Result.failure(IllegalArgumentException("Unknown Multicodec code: ${'$'}code"))
            return Result.success(name)
        }

        fun nameToType(name: String): Result<Multicodec> {
            val type = nameToType[name] ?: return Result.failure(IllegalArgumentException("Unknown Multicodec name: ${'$'}name"))
            return Result.success(type)
        }

        fun codeToType(code: Int): Result<Multicodec> {
            val type = codeToType[code] ?: return Result.failure(IllegalArgumentException("Unknown Multicodec code: ${'$'}code"))
            return Result.success(type)
        }
    }""",
    )
