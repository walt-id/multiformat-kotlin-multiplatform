...
class CidV0(multihash: Multihash) : Cid(multihash) {
    ...
    override fun toV1(): Result<Cid> {
        return Result.success(CidV1(multihash, Multicodec.DAG_PB, Multibase.BASE58_BTC))
    }
    ...
    override fun toString(base: Multibase): Result<String> {
        if (base != Multibase.BASE58_BTC) {
            return Result.failure(IllegalArgumentException("invalid base encoding"))
        }
        return Result.success(Base58.encodeToStringBtc(bytes()))
    }
