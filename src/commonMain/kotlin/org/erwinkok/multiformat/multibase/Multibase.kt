...
enum class Multibase(
    val code: String,
    val encoding: String,
    private val encoder: (ByteArray) -> String,
    private val decoder: (String) -> Result<ByteArray>,
) {
    ...
    companion object {
        ...
        fun nameToBase(name: String): Result<Multibase> {
            val base = nameToBase[name] ?: return Result.failure(IllegalArgumentException("Unknown Multibase name: $name"))
        ...
        fun encoding(data: String): Result<Multibase> {
            if (data.isEmpty()) {
                return Result.failure(IllegalArgumentException("cannot decode Multibase for empty string"))
            }
            val baseIndex = decodeBase(data)
            val base = codeToBase[baseIndex] ?: return Result.failure(IllegalArgumentException("Could not determine Multibase from: $data"))
            return Result.success(base)
        }
