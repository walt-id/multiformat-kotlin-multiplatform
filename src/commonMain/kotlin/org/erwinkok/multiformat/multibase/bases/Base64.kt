...
class Base64(private val encode: String, private val padChar: Char = StdPadding) {
    ...
    fun encodeToString(src: ByteArray): String {
        ...
        while (si < n) {
            ...
            val v =
                ((src[si + 0]).toUByte().toUInt() shl 16) or ((src[si + 1]).toUByte().toUInt() shl 8) or (src[si + 2]).toUByte().toUInt()
    ...
    fun decodeString(data: String): Result<ByteArray> {
        ...
        for (i in 0 until end) {
            ...
            if (decodeMap[r.code] == (-1).toByte()) {
                return Result.failure(IllegalArgumentException("invalid alphabet digit ($r)"))
            }
        ...
        if (bits >= 6 || 0xff and (buffer shl (8 - bits)) != 0) {
            return Result.failure(IllegalArgumentException("Unexpected end of data"))
        }
