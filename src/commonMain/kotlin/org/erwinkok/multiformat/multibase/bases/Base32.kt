...
class Base32(private val encode: String, private val padChar: Char = StdPadding) {
    ...
    fun encodeToString(src: ByteArray): String {
        ...
        while (srcIndex < (src.size - 4)) {
            ...
            dst[dstIndex + 3] =
                encode[((src[srcIndex + 2].toUByte().toInt() ushr 4) or (src[srcIndex + 1].toUByte().toInt() shl 4)) and 0x1F]
            dst[dstIndex + 2] = encode[(src[srcIndex + 1].toUByte().toInt() ushr 1) and 0x1F]
            dst[dstIndex + 1] =
                encode[((src[srcIndex + 1].toUByte().toInt() ushr 6) or (src[srcIndex + 0].toUByte().toInt() shl 2)) and 0x1F]
    ...
    private fun decode(dst: ByteArray, src: ByteArray): Result<Int> {
        ...
        while (left > 0 && !end) {
            ...
            while (j < 8) {
                if (left == 0) {
                    if (padChar != NoPadding) {
                        return Result.failure(IllegalArgumentException("illegal base32 data at input byte ${olen - left - j}"))
                    }
                    dlen = j
                    break
                }
                ...
                if (inp == padChar.code.toByte() && j >= 2 && left < 8) {
                    if (padChar == NoPadding) {
                        return Result.failure(IllegalArgumentException("illegal base32 data at input byte $olen"))
                    }
                    ...
                    for (k in 0 until 8 - 1 - j) {
                        if (left > k && src[srcIndex + k] != padChar.code.toByte()) {
                            // incorrect padding
                            return Result.failure(IllegalArgumentException("illegal base32 data at input byte ${olen - left + k - 1}"))
                        }
                    }
                    dlen = j
                    end = true
                    ...
                    if (dlen == 1 || dlen == 3 || dlen == 6) {
                        return Result.failure(IllegalArgumentException("illegal base32 data at input byte ${olen - left - 1}"))
                    }
                    break
                }
            ...
            if (dlen >= 7) {
                dst[dstIndex + 3] =
                    ((dbuf[4].toUByte().toInt() shl 7) or (dbuf[5].toUByte().toInt() shl 2) or (dbuf[6].toUByte().toInt() ushr 3)).toByte()
            }
            if (dlen >= 5) {
            ...
            if (dlen >= 4) {
                dst[dstIndex + 1] =
                    ((dbuf[1].toUByte().toInt() shl 6) or (dbuf[2].toUByte().toInt() shl 1) or (dbuf[3].toUByte().toInt() ushr 4)).toByte()
            }
            if (dlen >= 2) {
    ...
    fun decodeString(s: String): Result<ByteArray> {
        ...
        return Result.success(strb.copyOfRange(0, n))
    }
