    ...
    fun decodeString(input: String): Result<ByteArray> {
        ...
        for (r in input) {
            if ((r > maxDigitOrdinal) || revAlphabet[r.code] > maxDigitValueB36) {
                return Result.failure(IllegalArgumentException("invalid base36 digit ($r)"))
            }
        ...
        for (msb in zcount until outidx) {
            if (binu[msb].toUByte() > 0u) {
                return Result.success(binu.copyOfRange(msb - zcount, outidx))
            }
        }
        return Result.success(binu.copyOfRange(0, outidx))
    }
}
