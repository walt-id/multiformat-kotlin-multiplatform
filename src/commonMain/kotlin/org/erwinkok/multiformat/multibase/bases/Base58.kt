...
class Base58(private val encode: String) {
    ...
    fun decode(input: String): Result<ByteArray> {
        if (input.isEmpty()) {
            return Result.failure(IllegalArgumentException("can not decode zero-length string"))
        }
        val zero = encode[0]
        ...
        for (r in input) {
            if (r.code > 127) {
                return Result.failure(IllegalArgumentException("high-bit set on invalid digit"))
            }
            if (decode[r.code] == (-1).toByte()) {
                return Result.failure(IllegalArgumentException("invalid base58 digit ($r)"))
            }
        ...
        for (msb in zcount until binu.size) {
            if (binu[msb].toUByte() > 0u) {
                return Result.success(binu.copyOfRange(msb - zcount, outLen))
            }
        }
        return Result.success(binu.copyOfRange(0, outLen))
    }
