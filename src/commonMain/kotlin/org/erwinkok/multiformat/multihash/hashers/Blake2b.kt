    ...
    companion object {
        ...
        fun fromKey(key: ByteArray?, hashSize: Int = Size): Result<Blake2b> {
            if (hashSize < 1 || hashSize > Size) {
                return Result.failure(IllegalArgumentException("Unsupported hashSize $hashSize in Blake2b"))
            }
            if ((key != null) && (key.size > Size)) {
                return Result.failure(IllegalArgumentException("Unsupported keySize ${key.size} in Blake2b"))
            }
            return Result.success(Blake2b(key, hashSize))
        }
