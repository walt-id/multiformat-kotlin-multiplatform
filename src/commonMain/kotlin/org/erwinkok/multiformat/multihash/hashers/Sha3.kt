...
open class Sha3(
    ...
    override fun write(p: ByteArray): Result<Int> {
        if (state != SpongeDirection.SpongeAbsorbing) {
            return Result.failure(IllegalArgumentException("sha3: write to sponge after read"))
        }
        var index = 0
    ...
    fun read(out: ByteArray): Result<Int> {
        ...
        return Result.success(out.size)
    }
    ...
    fun clone(): Sha3 {
        ...
        a.copyInto(out.a) // was System.arraycopy(a, 0, out.a, 0, a.size)
        buf.copyInto(out.buf) // was System.arraycopy(buf, 0, out.buf, 0, buf.size)
        out.state = state
