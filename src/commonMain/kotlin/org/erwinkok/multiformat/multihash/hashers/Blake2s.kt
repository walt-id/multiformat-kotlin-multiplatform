...
class Blake2s private constructor(key: ByteArray?) : Hasher {
    ...
    init {
        if (key != null) {
            ...
            key.copyInto(this.key) //this.key = key.copyOf()
            //key.copyInto(this.key) // was System.arraycopy(key, 0, this.key, 0, key.size)
        }
        reset()
    }
    ...
    override fun write(p: ByteArray): Result<Int> {
        ...
        if (offset > 0) {
            ...
            if (n <= remaining) {
                p.copyInto(block, destinationOffset = offset)
                //System.arraycopy(p, 0, block, offset, p.size)

                offset += p.size
                return Result.success(n)
            }
            p.copyInto(block, destinationOffset = offset, endIndex = remaining)
...
//        p.copyInto(block, startIndex = index, endIndex = p.size - index)
    ...
    override fun reset() {
        ...
        if (keyLen > 0) {
            ...
            //key.copyInto(block) // was System.arraycopy(key, 0, block, 0, key.size)
            key.copyInto(block)
    ...
    private fun finalize(hash: ByteArray) {
        ...
        block.copyInto(blk, endIndex = offset)
    ...
    private fun hashBlocks(h: UIntArray, c: UIntArray, flag: UInt, blocks: ByteArray, index: Int, size: Int) {
        ...
        while (i < size) {
            ...
            var v15 = iv[7]
            for (j in m.indices) {
                m[j] = blocks[i].toUByte().toUInt() or (blocks[i + 1].toUByte().toUInt() shl 8) or (blocks[i + 2].toUByte()
                    .toUInt() shl 16) or (blocks[i + 3].toUByte().toUInt() shl 24)
                i += 4
            }
    ...
    companion object {
        ...
        fun fromKey(key: ByteArray?): Result<Blake2s> {
            if ((key != null) && (key.size > Size)) {
                return Result.failure(IllegalArgumentException("Unsupported keySize ${key.size} in Blake2s"))
            }
            return Result.success(Blake2s(key))
        }
