        ...
        fun readFrom(stream: CustomStream): Result<Pair<Protocol, ByteArray>> {
            ...
            if (protocol.size != 0) {
                ...
                if (stream.available() < size || size < 0) {
                    return Result.failure(IllegalArgumentException("invalid value for size " + stream.available()))
                }
            ...
            return Result.success(Pair(protocol, addressBytes))
        }

        fun readFrom(list: MutableList<String>): Result<Pair<Protocol, String>> {
            val part = list.removeFirst()
            val protocol = Protocol.withName(part) ?: return Result.failure(IllegalArgumentException("no protocol with name $part"))
            val addressString = if (protocol.size != 0) {
                if (list.isEmpty()) {
                    return Result.failure(IllegalArgumentException("unexpected end of Multiaddress"))
                }
                if (protocol.path) {
