...
internal class CidTest {
    ...
    fun throwsOnInvalidBS58StrMultihash() {
        expectError("input isn't valid multihash") { Cid.fromString("QmdfTbBqBPQ7VNxZEYEj14VmRuZBkqFbiwReogJgS1zIII") }
    }

    @Test
    fun throwsOnTryingToCreateACIDv0WithACodecOtherThanDagpb() {
        val hash = createHashSha256("abc")
        expectError("codec must be 'dag-pb' for CIDv0") {
            Cid.builder()
                .withVersion(0)
    ...
    fun throwsOnTryingToCreateACIDv0WithABaseOtherThanBase58btc() {
        val hash = createHashSha256("abc")
        expectError("multibase must be 'base58btc' for CIDv0") {
            Cid.builder()
                .withVersion(0)
    ...
    fun v0Prefix() {
        ...
        val cid = Cid.builder()
            ...
            .build()
            .expectNoErrors()
        assertEquals("00701220", cid.prefix().bytes().toHexString())
    }
    ...
    fun v0Bytes() {
        ...
        val cid = Cid.builder()
            ...
            .build()
            .expectNoErrors()
        assertEquals("1220ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad", cid.bytes().toHexString())
    }
    ...
    fun handlesCIDNoMultibase() {
        ...
        val cidBuf = "017012207252523e6591fb8fe553d67ff55a86f84044b46a3e4176e10c58fa529a4aabd5".hexToByteArray()
    ...
    fun handlesMultibyteVarintEncodedCodecCodes() {
        val ethBlockHash = "8a8e84c797605fbe75d5b5af107d4220a2db0ad35fd66d9be3d38d87c472b26d".hexToByteArray()
    ...
    fun v1Prefix() {
        ...
        val cid = Cid.builder()
            ...
            .build()
            .expectNoErrors()
        assertEquals("01711220", cid.prefix().bytes().toHexString())
    }
    ...
    fun v1bytes() {
        ...
        val cid = Cid.builder()
            ...
            .build()
            .expectNoErrors()
        assertEquals("01711220ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad", cid.bytes().toHexString())
    }
    ...
    @Test
    fun emptyString() {
        expectError("cid too short") { Cid.fromString("") }
    }
    ...
    fun `prefix v1`() {
        ...
        val prefix =
            Prefix(1, Multicodec.DAG_CBOR, Multicodec.SHA2_256, MultihashRegistry.defaultHashSize(Multicodec.SHA2_256).expectNoErrors())
    ...
    fun `prefix v0`() {
        val data = "this is some test content".toByteArray()
        val prefix =
            Prefix(0, Multicodec.DAG_PB, Multicodec.SHA2_256, MultihashRegistry.defaultHashSize(Multicodec.SHA2_256).expectNoErrors())
        val cid1 = prefix.sum(data).expectNoErrors()
        assertEquals(prefix, cid1.prefix())
        val hash = Multihash.sum(Multicodec.SHA2_256, data).expectNoErrors()
        val cid2 = Cid.builder()
    ...
    fun `bad prefix`() {
        ...
        expectError("invalid cid version") { prefix.sum(byteArrayOf(0x00, 0x01, 0x03)) }
    }
    ...
    fun `bad prefix from bytes`() {
        expectError("UnexpectedEndOfStream") { Prefix.fromBytes(byteArrayOf(0x80.toByte())) }
        expectError("UnexpectedEndOfStream") { Prefix.fromBytes(byteArrayOf(0x01, 0x80.toByte())) }
        expectError("UnexpectedEndOfStream") { Prefix.fromBytes(byteArrayOf(0x01, 0x01, 0x80.toByte())) }
        expectError("UnexpectedEndOfStream") { Prefix.fromBytes(byteArrayOf(0x01, 0x01, 0x01, 0x80.toByte())) }
    }
