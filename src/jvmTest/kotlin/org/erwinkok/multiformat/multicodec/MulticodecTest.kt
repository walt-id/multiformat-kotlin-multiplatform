...
internal class MulticodecTest {
    ...
    fun `generates error when getting type from unknown multicodec name`() {
        expectError("Unknown Multicodec name: this-codec-doesnt-exist") { Multicodec.nameToType("this-codec-doesnt-exist") }
    }

    @Test
    fun `generates error when getting type from unknown multicodec type`() {
        expectError("Unknown Multicodec code: 65518") { Multicodec.codeToType(0xffee) }
    }
}
