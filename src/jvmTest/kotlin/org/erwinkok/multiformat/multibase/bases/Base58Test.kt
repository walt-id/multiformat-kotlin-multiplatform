...
internal class Base58Test {
    ...
    @Test
    fun empty() {
        expectError("can not decode zero-length string") { Base58.decodeStringBtc("") }
    }

    @Test
    fun `invalid character`() {
        expectError("invalid base58 digit (*)") { Base58.decodeStringBtc("*&%^") }
    }

    @Test
    fun `high bit`() {
        expectError("high-bit set on invalid digit") { Base58.decodeStringBtc("😂") }
    }
}
