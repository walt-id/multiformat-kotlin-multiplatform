...
data class Tuple4(
    val hex: String, val code: Int, val name: String, val error: String,
)
...
internal class MultihashTest {
    ...
    fun base58String() {
        ...
        val expected = "122013bf801597d74a660453412635edd8c34271e5998f801fac5d700c6ce8d8e461".hexToByteArray()
    ...
    @Test
    fun encodeName() {
        val digest = "0beec7b5ea3f0fdbc95d0dd47f3c5bc275da8a33".hexToByteArray()
        val mh = Multihash.encodeName(digest, "sha1")
            .expectNoErrors()
        assertEquals("11140beec7b5ea3f0fdbc95d0dd47f3c5bc275da8a33", mh.bytes().toHexString())
        val mh2 = Multihash.fromBytes(mh.bytes())
            .expectNoErrors()
        assertEquals("sha1", mh2.name)
        assertEquals(0x11, mh2.code)
        assertEquals(20, mh2.length)
        assertEquals("0beec7b5ea3f0fdbc95d0dd47f3c5bc275da8a33", mh2.digest.toHexString())
    }
    ...
    fun decode(): Stream<DynamicTest> {
        return arguments.map { (hex, code, name, error) ->
            DynamicTest.dynamicTest(name) {
                ...
                expectError(error) { Multihash.fromBytes(digest) }
            }
        }.stream()
    }
    ...
    fun hex(): Stream<DynamicTest> {
        return arguments.map { (hex, code, name, _) ->
            DynamicTest.dynamicTest(name) {
                ...
                stream.writeUnsignedVarInt(digest.size)
                val multihashBytes = stream.toByteArray() + digest
                val hexBytes = multihashBytes.toHexString()
    ...
    fun decodeErrorInvalid() {
        expectError("input isn't valid multihash") { Multihash.fromBase58("/ipfs/QmQTw94j68Dgakgtfd45bG3TZG6CAfc427UVRH4mugg4q4") }
    }
    ...
    fun badVarint() {
        ...
        expectError("UnexpectedEndOfStream") { Multihash.fromBytes(byteArrayOf(129.toByte(), 128.toByte(), 128.toByte())) }
        expectError("varint not minimally encoded") { Multihash.fromBytes(byteArrayOf(129.toByte(), 0.toByte())) }
        expectError("varint not minimally encoded") { Multihash.fromBytes(byteArrayOf(128.toByte(), 0.toByte())) }
    }
    ...
    companion object {
        var arguments = listOf(
            ...
            Tuple4(
                ...
                0x14,
                "sha3-512",
                "Unknown Multicodec code: 75",
            ),
            Tuple4("d41d8cd98f00b204e9800998ecf8427e", 0xd5, "md5", "Unknown Multicodec code: 3796"),
            Tuple4(
                "14fcb37dc45fa9a3c492557121bd4d461c0db40e5dcfcaa98498bd238486c307",
                0x1012,
