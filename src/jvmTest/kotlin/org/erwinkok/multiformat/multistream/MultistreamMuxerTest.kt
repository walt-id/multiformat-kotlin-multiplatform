...
import org.erwinkok.suspendExpectError
...
internal class MultistreamMuxerTest {
    ...
    fun selectOneOfNA() = runTest {
        ...
        require(selectOneResult.exceptionOrNull()?.message == "Peer does not support any of the given protocols")
    ...
    fun selectFails() = runTest {
        ...
        suspendExpectError("Peer does not support any of the given protocols") { MultistreamMuxer.selectOneOf(setOf(ProtocolId.of("/d"), ProtocolId.of("/e")), remoteConnection) }
    ...
    fun simOpenClientServerFail() = runTest {
        muxer.addHandler(ProtocolId.of("/a"))
        launch {
            suspendExpectError("end negotiating: we do not support any of the requested protocols") { muxer.negotiate(localConnection) }
        }
        suspendExpectError("Peer does not support any of the given protocols") { MultistreamMuxer.selectWithSimopenOrFail(setOf(ProtocolId.of("/b")), remoteConnection) }
        remoteConnection.close()
    }
    ...
    fun simOpenClientClientFail() = runTest {
        val job = launch {
            suspendExpectError("Peer does not support any of the given protocols") { MultistreamMuxer.selectWithSimopenOrFail(setOf(ProtocolId.of("/a")), remoteConnection) }
            remoteConnection.close()
        }
        suspendExpectError("Peer does not support any of the given protocols") { MultistreamMuxer.selectWithSimopenOrFail(setOf(ProtocolId.of("/b")), localConnection) }
        localConnection.close()
        job.join()
    }
